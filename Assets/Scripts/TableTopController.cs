...
public class TableTopController : MonoBehaviour {
	...
	void Start () {
		...
		// sprite.triangles = new int[] { 0, 1, 2, 0, 2, 3 };
	...
	private bool LoadSettings() {
		Debug.Log ("Loading CITY keystone settings from " + _settingsFileName);
	...
	private bool SaveSettings() {
		Debug.Log ("Saving CITY keystone settings in " + _settingsFileName);
	...
	private void UpdateSelection() {
		...
		var corner = Input.GetKeyDown ("5") ? 0 : (Input.GetKeyDown ("6") ? 1 : (Input.GetKeyDown ("7") ? 2 : (Input.GetKeyDown ("8") ? 3 : selectedCorner)));
		...
		if (corner != selectedCorner) {
			...
			if (_debug) 
				Debug.Log ("CITY Selection changed to " + selectedCorner.ToString ());
		}
