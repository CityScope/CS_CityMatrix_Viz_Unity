...
/// 
/// </summary>
public class InputSlider : InputLegoUI {
	...
	/// </summary>
	public InputSlider(GameObject parentObject, float _scannerScale) {
	...
	public bool UpdateSlider() {
		...
		for (int i = 0; i < sliderScanners.GetLength(1); i++) {
			int currId = GameObject.Find ("GridDecoder").GetComponentInChildren<InputScanners>().GetColorIdByCoord(0, i, ref sliderScanners, false);
			if (currIds[i] != currId) {
				currIds [i] = currId;
				needsUpdate = true;
			}
	...
	private void RecomputeSliderValue (ref List<int> refColorIndex) {
		...
		if (this.value != newValue) {
			...
			// GameObject.Find ("GridDecoder").GetComponentInChildren<InputScanners>().RefreshSliderUI(sliderID, this.value);
		}
